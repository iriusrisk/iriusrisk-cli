# Compare Versions - AI Assistant Guide
...
This tool compares different states of a threat model to identify architectural and security changes. You will receive **structured comparison results** (not raw files) that show what changed between versions, which you then interpret and communicate to humans.

**This is a DATA tool** - it provides structured diff results for you to analyze. For comprehensive CI/CD workflows that include control verification and orchestration, see `ci_cd_verification` tool instead.
